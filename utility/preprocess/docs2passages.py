...
def main(args):
    ...
    output_path = f'{args.input}_sliced_{args.nwords}_{args.overlap}.{letter}'
        ...
        for docid, title, url, passages in Collection:
            for passage in passages:
                if args.format == Format1:
                    ...
                    f.write('\t'.join([str(line_idx), title + " " + passage]) + '\n')
                    map.append((line_idx, docid))

                line_idx += 1

    with open('map_splitted.tsv', 'w') as f:
        for line_idx, docid in map:
            f.write('\t'.join([str(line_idx), docid]) + '\n')
...
if __name__ == "__main__":
    ...
    # Input Arguments.
    parser.add_argument('--input', dest='input', default='/gpfs/home3/sgarcarz/PycharmProjects/ColBERT/docs/downloads/msmarco_docs/collection_original.tsv')
    parser.add_argument('--format', dest='format', default=Format3)

    # Output Arguments.
    parser.add_argument('--use-wordpiece', dest='use_wordpiece', default=False, action='store_true')
    parser.add_argument('--nwords', dest='nwords', default=499, type=int)
    parser.add_argument('--overlap', dest='overlap', default=100, type=int)

    # Other Arguments.
    parser.add_argument('--nthreads', dest='nthreads', default=72, type=int)
